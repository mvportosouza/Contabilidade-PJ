...
export function getAnnualStatistics(
  ...
  const rows = Array.from({ length: 12 }, (_, i) => {
    ...
    return {
      ...
      isMelhor: false,
      isPior: false,
      ativo: receita > 0 || despesa > 0 || pl > 0 || distribuicao > 0,
    }
  })

  let receitaAcumulada = 0
  let mesesComReceita = 0
  rows.forEach((row) => {
    if (row.receita > 0) {
      receitaAcumulada += row.receita
      mesesComReceita += 1
    }
    row.mediaReceitaAcumulada = mesesComReceita > 0
      ? receitaAcumulada / mesesComReceita
      : 0
  })

  const active = rows.filter(row => row.ativo)
  if (active.length) {
