...
export function calcDAS(rbt12, receitaMes) {
  ...
  if (!receita) return { valor: 0, aliquota: 0, faixa: 0, rbt12: rbt };
...
export function calcRecommendedPL(transactions, plMap, year, month) {
  const { rbt12, rbt12Anualizado, mesesAtividade } = calcRBT12(transactions, year, month);
  const baseRBT = mesesAtividade > 0 && mesesAtividade < 12 ? rbt12Anualizado : rbt12;
  if (!baseRBT) return SALARIO_MINIMO_2026;
  ...
  for (let i = 1; i < 12; i += 1) {
    ...
    const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    const pl = Math.max(0, clampNumber(plMap?.[key]));
    previousFS += pl + calcINSS(pl);
  }

  const target = baseRBT * 0.28;
  if (previousFS >= target) return SALARIO_MINIMO_2026;

  let lo = 0;
  let hi = Math.max(SALARIO_MINIMO_2026, baseRBT * 0.28);
  for (let i = 0; i < 48; i += 1) {
    const mid = (lo + hi) / 2;
    const fs = previousFS + mid + calcINSS(mid);
    if (fs >= target) hi = mid;
    else lo = mid;
  }
