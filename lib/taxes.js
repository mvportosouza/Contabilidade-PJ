...
export function calcFatorR(transactions, plMap, year, month) {
  ...
  for (let offset = 0; offset < totalMonths; offset += 1) {
    ...
    // Regra proporcional dos primeiros 12 meses.
    if (monthsSinceStart === 1) {
      fs12 = proLaboreMes;
  ...
  const meta = Math.max(0, rbtData.rbt12 * FATOR_R_MINIMO);
  ...
  return {
    ...
    folhaProLabore12: round2(folhaProLabore12),
    cpp12: round2(cpp12),
...
export function calcRecommendedPL(transactions, plMap, year, month) {
  /*
   * Planejamento correto do pró-labore:
   *
   * O pró-labore pago no mês atual NÃO altera o Fator R do próprio PA
   * (exceto no mês de início de atividade). Ele entra na janela do PA
   * seguinte. Portanto, a recomendação deste mês deve responder:
   *
   * "Quanto preciso pagar neste mês para que o próximo PA tenha Fator R
   * >= 28%, considerando a folha que já tenho e a CPP efetivamente
   * estimada/recolhida?"
   *
   * Para empresas com menos de 13 meses, a regra de anualização multiplica
   * tanto a folha quanto a receita pela mesma razão; por isso, na solução
   * do pró-labore podemos trabalhar com os acumulados dos meses de atividade.
   * A partir do 13º mês, usamos a janela móvel dos 12 meses anteriores ao
   * próximo PA (isto é, o mês atual + os 11 meses anteriores).
   *
   * IMPORTANTE: o INSS de 11% do sócio NÃO é somado à FS12. A FS12 considera
   * remuneração/pró-labore e a contribuição patronal previdenciária (CPP),
   * além dos demais componentes legais de folha quando existirem. O INSS
   * retido do sócio é contribuição do segurado e não deve ser duplicado como
   * folha do Fator R.
   */
  ...
  const nextPA = addMonths(current, 1);
  const monthsSinceStartNext = monthDiff(firstMonth, nextPA) + 1;

  // Receita que será usada no RBT12 do próximo PA.
  let receitaBase = 0;
  if (monthsSinceStartNext <= 12) {
    // Nos primeiros 12 meses, a média/anualização cancela na razão
    // FS12/RBT12; usamos o acumulado dos meses já decorridos.
    for (let i = 0; i < monthsSinceStartNext - 1; i += 1) {
      const d = addMonths(nextPA, -1 - i);
      receitaBase += receitas[monthKey(d.getFullYear(), d.getMonth())] || 0;
    }
  } else {
    // A partir do 13º mês: 12 meses imediatamente anteriores ao próximo PA.
    for (let i = 1; i <= 12; i += 1) {
      const d = addMonths(nextPA, -i);
      receitaBase += receitas[monthKey(d.getFullYear(), d.getMonth())] || 0;
    }
  }

  receitaBase = Math.max(0, receitaBase);
  if (!receitaBase) return SALARIO_MINIMO_2026;

  // Folha que já estará na janela do próximo PA, sem o pró-labore deste mês.
  let folhaExistente = 0;
  const monthsToLookBack = Math.min(12, Math.max(0, monthsSinceStartNext - 1));

  for (let i = 1; i <= monthsToLookBack; i += 1) {
    const d = addMonths(nextPA, -i);
    const key = monthKey(d.getFullYear(), d.getMonth());

    // Não contar o mês atual: ele será resolvido pelo valor recomendado.
    if (d.getFullYear() === current.getFullYear() && d.getMonth() === current.getMonth()) {
      continue;
    }

    folhaExistente += getPayrollForMonth(plMap, d.getFullYear(), d.getMonth());
  }

  /*
   * CPP do mês atual.
   * O Anexo do PA atual é determinado pelo Fator R do próprio PA, que não
   * usa o pró-labore pago neste mês (salvo o mês inicial). Assim, podemos
   * obter a CPP do DAS atual sem criar uma dependência circular com o
   * pró-labore recomendado.
   */
  let cppAtual = 0;
  const receitaAtual = receitas[monthKey(current.getFullYear(), current.getMonth())] || 0;

  if (receitaAtual > 0) {
    const fatorAtual = calcFatorR(transactions, plMap, year, month);
    const anexoAtual = fatorAtual.fatorR >= FATOR_R_MINIMO ? 'III' : 'V';
    const tabelaAtual = anexoAtual === 'III'
      ? SIMPLES_ANEXO_III
      : SIMPLES_ANEXO_V;
    const rateAtual = effectiveRate(fatorAtual.rbt12, tabelaAtual);
    const dasAtual = receitaAtual * rateAtual.aliquota;
    cppAtual = dasAtual * rateAtual.cppPartilha;
  }

  /*
   * A meta é:
   *   folhaExistente + pró-laboreAtual + CPPAtual >= 28% × receitaBase
   *
   * A CPP é um componente adicional da folha. O INSS de 11% do sócio fica
   * fora desta equação.
   */
  const metaFolha = receitaBase * FATOR_R_MINIMO;
  const necessario = metaFolha - folhaExistente - cppAtual;

  return round2(Math.max(SALARIO_MINIMO_2026, necessario));
}
