...
export function calcRecommendedPL(transactions, plMap, year, month) {
  ...
  const paNext = addMonths(monthStart(year, month), 1);
  const rbtNext = calcRBT12(transactions, paNext.getFullYear(), paNext.getMonth()).rbt12;

  if (rbtNext <= 0) return SALARIO_MINIMO_2026;

  let fsWithoutCurrent = 0;
  for (let i = 1; i <= 12; i += 1) {
    const d = addMonths(paNext, -i);
    if (d.getFullYear() === year && d.getMonth() === month) continue;
    fsWithoutCurrent += getPayrollForMonth(plMap, d.getFullYear(), d.getMonth());
  }

  // Para não introduzir uma dependência circular no valor recomendado,
  // usamos o pró-labore como componente principal da folha. A CPP histórica
  // já está refletida nos períodos anteriores quando o fator R é apurado.
  const meta = rbtNext * FATOR_R_MINIMO;
  const recomendado = Math.max(
    SALARIO_MINIMO_2026,
    meta - fsWithoutCurrent,
  );

  return round2(recomendado);
}
