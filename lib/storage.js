...
async function syncQueue(userId = loadedUserId, { force = false } = {}) {
  ...
  if (syncInProgress) {
    // A sync may already be running because AuthGate restored the session.
    // Waiting only for that first sync is not enough: the current operation
    // may have queued a newer snapshot while the first request was in flight.
    // After the in-flight request finishes, re-enter syncQueue so the newest
    // queued snapshot is actually persisted before the caller is released.
    ...
    if (
      ...
      readSyncQueue(userId)?.state
    ) {
      return syncQueue(userId, { force })
    }
...
/**
 * Replaces the complete application state as one logical operation.
 *
 * Offline: persist the whole snapshot locally and queue it for synchronization.
 * Online: write the snapshot remotely first and only then commit it locally,
 * so a failed remote write cannot leave a partially restored local state.
 */
export async function replaceState(nextState) {
  if (typeof window === 'undefined') return false

  const operation = saveChain.then(async () => {
    const previousState = normalizeState(await loadState())
    const replacement = normalizeState(nextState)
    const user = await currentUser()
    ...
    const baseUpdatedAt = previousEnvelope.remoteUpdatedAt || remoteUpdatedAt || null

    if (typeof navigator !== 'undefined' && navigator.onLine === false) {
      stateCache = replacement
      loadedUserId = user.id
      ...
      writeSyncQueue(user.id, replacement, baseUpdatedAt)

      setStatus({ state: 'offline', dirty: true, conflict: false, error: null })
      return true
    }

    const result = await saveStateToSupabase(user.id, replacement, baseUpdatedAt)

    if (!result.ok) {
      // Transactional guarantee: a failed remote restore does not replace the
      // previously committed local snapshot.
      stateCache = previousState
      loadedUserId = user.id
      remoteUpdatedAt = baseUpdatedAt

      setStatus({
        state: result.conflict ? 'conflict' : 'error',
        dirty: Boolean(previousEnvelope.dirty),
        conflict: Boolean(result.conflict),
        error: result.conflict
          ? 'Existe uma alteração mais recente na nuvem. Os dados locais foram preservados.'
          : 'Não foi possível salvar o estado restaurado.',
      })
      ...
      if (result.error) {
        ...
        throw new Error(
          result.conflict
            ? `Não foi possível restaurar o estado: existe uma versão mais recente na nuvem. ${reason}`
            : `Não foi possível restaurar/aplicar o estado remoto: ${reason}`
        )
      }

      throw new Error(
        result.conflict
          ? 'Não foi possível restaurar o estado: existe uma versão mais recente na nuvem.'
          : 'Não foi possível restaurar/aplicar o estado remoto.'
      )
    }
    ...
    loadedUserId = user.id
    remoteUpdatedAt = result.updatedAt
    writeLocalEnvelope(user.id, envelope)
    removeSyncQueue(user.id)
    conflictSnapshot = null
  ...
  saveChain = operation.catch((error) => {
    console.error('Erro ao restaurar o estado:', error)
    setStatus({ state: 'error', dirty: true, error: 'Não foi possível restaurar o estado.' })
  })
