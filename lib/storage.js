...
function readLocalEnvelope(userId) {
  ...
  try {
    ...
    logger.error('database.local_cache_read_failed', { category: 'database', operation: 'local_cache_read', code: error?.code, status: error?.status })
    return null
  }
}
...
function writeLocalEnvelope(userId, envelope) {
  ...
  try {
    ...
    logger.error('database.local_cache_write_failed', { category: 'database', operation: 'local_cache_write', code: error?.code, status: error?.status })
...
function readSyncQueue(userId) {
  ...
  try {
    ...
    logger.error('database.sync_queue_read_failed', { category: 'database', operation: 'sync_queue_read', code: error?.code, status: error?.status })
    return null
  }
}
...
function writeSyncQueue(userId, state, baseUpdatedAt = null) {
  ...
  try {
    ...
    logger.error('database.sync_queue_write_failed', { category: 'database', operation: 'sync_queue_write', code: error?.code, status: error?.status })
...
function removeSyncQueue(userId) {
  ...
  try {
    ...
    logger.error('database.sync_queue_remove_failed', { category: 'database', operation: 'sync_queue_remove', code: error?.code, status: error?.status })
  }
}
...
function removeLocalState(userId) {
  ...
  try {
    ...
    logger.error('database.local_cache_clear_failed', { category: 'database', operation: 'local_cache_clear', code: error?.code, status: error?.status })
  }
}
...
async function getSession() {
  try {
    ...
    if (error) {
      logger.error('auth.session_restore_failed', { category: 'auth', operation: 'session_restore', code: error?.code, status: error?.status })
      return null
    }
...
async function saveStateToSupabase(userId, state, baseUpdatedAt = null, { force = false } = {}) {
  ...
  try {
    ...
    logger.error('database.sync_failed', { category: 'database', operation: 'sync', code: error?.code, status: error?.status })
...
async function loadStateInternal() {
  ...
  try {
    ...
    logger.error('database.load_failed', { category: 'database', operation: 'load', code: error?.code, status: error?.status })
...
export async function sSet(key, value) {
  ...
  const operation = saveChain.then(async () => {
    ...
    /*
     * IMPORTANT for multi-tab concurrency:
     * localStorage is shared by every tab, so its envelope may already
     * contain a newer timestamp written by another tab. The optimistic
     * concurrency base must come from this tab's in-memory snapshot
     * (`remoteUpdatedAt`), otherwise tab B could silently adopt tab A's
     * timestamp and overwrite A instead of receiving a conflict.
     *
     * When this tab has not loaded a remote snapshot yet, fall back to
     * the shared envelope (e.g. first load/offline recovery).
     */
    const baseUpdatedAt =
      remoteUpdatedAt ||
      envelope?.remoteUpdatedAt ||
      null
    ...
    writeSyncQueue(user.id, state, baseUpdatedAt)
    ...
    // Another browser event (focus/visibility/online) can legitimately start
    // a sync between the write and the call above. syncQueue intentionally
    // serializes those operations; wait for that in-flight operation and make
    // one deterministic retry so a successful write is not left dirty merely
    // because a concurrent sync was already running.
    if (syncInProgress) {
      while (syncInProgress) {
        await new Promise(resolve => setTimeout(resolve, 25))
      }
      if (readSyncQueue(user.id)?.state) {
        await syncQueue(user.id)
      }
    }
  })
  ...
  saveChain = operation.catch((error) => {
    logger.error('database.save_failed', { category: 'database', operation: 'save', code: error?.code, status: error?.status })
...
export async function replaceState(nextState) {
  if (typeof window === 'undefined') throw new Error('Armazenamento indisponível.');

  // Serialize the whole restore with ordinary saves. Nothing is exposed to
  // React until the complete replacement has been durably prepared.
  await saveChain;

  const user = await currentUser();
  if (!user?.id) throw new Error('Sessão indisponível para restaurar o backup.');

  const normalizedNext = normalizeState(nextState);
  const previousState = normalizeState(stateCache || await loadState());
  const previousEnvelope = readLocalEnvelope(user.id);
  const previousQueue = readSyncQueue(user.id);
  const previousRemoteUpdatedAt = remoteUpdatedAt;

  const operation = (async () => {
    const baseUpdatedAt = previousRemoteUpdatedAt || previousEnvelope?.remoteUpdatedAt || null;

    // Stage the new state locally first. The queue is the durable source of
    // truth while a cloud commit is pending. StateCache is changed only after
    // both local writes succeed.
    if (!writeSyncQueue(user.id, normalizedNext, baseUpdatedAt)) {
      throw new Error('Não foi possível preparar o restore no armazenamento local.');
    }
    if (!writeLocalEnvelope(user.id, {
      state: normalizedNext,
      remoteUpdatedAt: baseUpdatedAt,
      localUpdatedAt: nowIso(),
      dirty: true,
    })) {
      // Queue was staged but the envelope failed: restore the exact previous
      // local durable state before exposing any change.
      if (previousQueue) writeSyncQueue(user.id, previousQueue.state, previousQueue.baseUpdatedAt);
      else removeSyncQueue(user.id);
      if (previousEnvelope) writeLocalEnvelope(user.id, previousEnvelope);
      else removeLocalState(user.id);
      throw new Error('Não foi possível preparar o restore no armazenamento local.');
    }

    if (typeof navigator !== 'undefined' && navigator.onLine === false) {
      stateCache = normalizedNext;
      loadedUserId = user.id;
      remoteUpdatedAt = baseUpdatedAt;
      setStatus({ state: 'offline', dirty: true, conflict: false, error: null });
    ...
    const result = await saveStateToSupabase(user.id, normalizedNext, baseUpdatedAt);
    if (!result.ok) {
      // No cloud commit occurred (conflict/failure), so restore the exact
      // previous local state. The application remains unchanged.
      if (previousQueue) writeSyncQueue(user.id, previousQueue.state, previousQueue.baseUpdatedAt);
      else removeSyncQueue(user.id);
      if (previousEnvelope) writeLocalEnvelope(user.id, previousEnvelope);
      else removeLocalState(user.id);
      ...
      if (result.conflict) {
        conflictSnapshot = { state: result.remoteState, updatedAt: result.remoteUpdatedAt };
        setStatus({ state: 'conflict', dirty: Boolean(previousQueue || previousEnvelope?.dirty), conflict: true, error: 'Existe uma alteração mais recente na nuvem. O restore não foi aplicado.' });
      } else {
        setStatus({ state: 'error', dirty: Boolean(previousQueue || previousEnvelope?.dirty), conflict: false, error: 'Não foi possível aplicar o restore.' });
      }
      throw new Error(result.conflict
        ? 'Restore rejeitado: existe uma versão mais recente na nuvem.'
        : 'Não foi possível aplicar o restore na nuvem.');
    }
    ...
    if (!writeLocalEnvelope(user.id, committedEnvelope)) {
      // Compensating transaction: the cloud row was committed, but the local
      // durable commit failed. Restore the cloud snapshot before surfacing an
      // error. This keeps the two storage representations aligned whenever
      // the browser permits the rollback write.
      const rollback = await saveStateToSupabase(user.id, previousState, result.updatedAt, { force: true });
      if (!rollback.ok) {
        throw new Error('Restore interrompido: falha ao confirmar e reverter o estado. Nenhum novo estado foi exposto no aplicativo.');
      }
      if (previousQueue) writeSyncQueue(user.id, previousQueue.state, previousQueue.baseUpdatedAt);
      else removeSyncQueue(user.id);
      if (previousEnvelope) writeLocalEnvelope(user.id, previousEnvelope);
      ...
      loadedUserId = user.id;
      remoteUpdatedAt = previousRemoteUpdatedAt;
      throw new Error('Restore não concluído: falha ao gravar o estado local.');
    }
  ...
  saveChain = operation.catch((error) => {
    logger.error('database.restore_failed', { category: 'database', operation: 'restore', code: error?.code, status: error?.status });
    return undefined;
  });
...
export async function deleteAllAppData() {
  ...
  // This action intentionally keeps the Auth account. It deletes only the
  // authenticated user's app_state row and then clears local representations.
  try {
    await saveChain
  } catch {
    // Continue with the explicit deletion even if a previous save failed.
  }
...
export async function clearStorageCache({
  ...
  if (clearLocal && typeof window !== 'undefined') {
    ...
    // Remove Supabase Auth persistence as a second local defense. The normal
    // signOut() path also clears it, but account deletion must not depend on
    // the remote sign-out endpoint remaining available after deleteUser().
    try {
      ...
      logger.error('auth.local_session_clear_failed', { category: 'auth', operation: 'local_session_clear', code: error?.code, status: error?.status })
    }
  }

  if (clearCaches && typeof window !== 'undefined' && 'caches' in window) {
    try {
      const cacheNames = await window.caches.keys()
      await Promise.all(cacheNames.map((cacheName) => window.caches.delete(cacheName)))

      // Ask the active Service Worker to perform the same purge from its
      // context, covering caches opened immediately before this operation.
      window.navigator.serviceWorker?.controller?.postMessage({
        type: 'CLEAR_CACHES',
      })
    } catch (error) {
      logger.error('auth.cache_clear_failed', { category: 'auth', operation: 'cache_clear', code: error?.code, status: error?.status })
    }
  }
