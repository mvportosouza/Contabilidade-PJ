...
export function generateMonthlyReportPdf({
  ...
  const sorted = [...transactions].sort((a,b) => String(b.data).localeCompare(String(a.data)));
  const txLines = ["LANCAMENTOS DO MES"];
  if (!sorted.length) txLines.push("Nenhum lancamento registrado neste mes.");
  sorted.forEach((tx) => {
    const tipo = tx.tipo === "receita" ? "RECEITA" : tx.tipo === "distribuicao" ? "DISTRIBUICAO" : "DESPESA";
    txLines.push(`${tx.data || "-"} | ${tipo} | ${money(tx.valor)} | ${tx.nome || tx.categoria || "-"}`);
  });
  ...
  return makePdf([
    pageForMonthly(
      ...
      "Resumo + graficos",
      (c) => {
        textRows(c, baseLines, 760);
        pieChart(c, 48, 382, 245, 190, [
    ...
    () => {
      ...
      const clinicItems = Object.entries(byClinic)
        .sort((a,b)=>b[1]-a[1])
        .map(([label,value])=>({label,value,color:label.startsWith("Ortodontia")?COLORS.gold:label.startsWith("Endodontia")?COLORS.navyMid:COLORS.muted}));
      reportHeader(c, `RELATORIO MENSAL - ${ascii(monthLabel).toUpperCase()} ${year}`, "Receita por clinica + lancamentos");
      horizontalBars(c, 42, 500, 511, 250, clinicItems.slice(0, 9), "Receita por Clinica");
      textRows(c, txLines, 460);
      return c;
    },
  ]);
}
