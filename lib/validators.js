...
export function normalizeTransaction(tx, { index = 0, imported = false, usedIds = new Set() } = {}) {
  ...
  const id = optionalString(tx.id, `Lançamento ${index + 1}: id` ).trim();
  if (!id) throw new Error(`Lançamento ${index + 1}: id ausente.`);
  if (usedIds.has(id)) throw new Error(`Lançamento ${index + 1}: id duplicado.`);
  usedIds.add(id);
...
export function normalizeBackup(payload) {
  ...
  if (payload.schema !== 'contabilidade-pj-backup') {
  ...
  if (payload.exportedAt == null && payload.at == null) {
