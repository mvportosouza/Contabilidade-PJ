...
const TRANSACTION_KEYS = new Set([
  ...
  'beneficiarioNome', 'beneficiarioCpf', 'pjCnpj', 'origemLucro',
  'aprovacaoDistribuicao', 'pagamentoPrevistoOriginal', 'irrfDistribuicao', 'valorLiquidoDistribuicao',
]);
...
const BACKUP_KEYS = new Set([
  ...
  'plMap', 'proLaboreMap', 'plManual', 'ctbMap', 'contabMap', 'irrfMap', 'pfOtherIncomeMap',
  'exportedAt', 'at',
]);
...
export function normalizeTransaction(tx, { index = 0, imported = false, usedIds = new Set() } = {}) {
  ...
  return {
    ...
    beneficiarioNome: optionalString(tx.beneficiarioNome, `Lançamento ${index + 1}: beneficiarioNome`),
    beneficiarioCpf: optionalString(tx.beneficiarioCpf, `Lançamento ${index + 1}: beneficiarioCpf`),
    pjCnpj: optionalString(tx.pjCnpj, `Lançamento ${index + 1}: pjCnpj`),
    origemLucro: optionalString(tx.origemLucro, `Lançamento ${index + 1}: origemLucro`) || (tx.tipo === "distribuicao" ? "apurados_2026" : ""),
    aprovacaoDistribuicao: tx.aprovacaoDistribuicao == null || tx.aprovacaoDistribuicao === "" ? "" : normalizeDateOnly(tx.aprovacaoDistribuicao),
    pagamentoPrevistoOriginal: tx.pagamentoPrevistoOriginal == null || tx.pagamentoPrevistoOriginal === "" ? "" : normalizeDateOnly(tx.pagamentoPrevistoOriginal),
    irrfDistribuicao: tx.irrfDistribuicao == null ? 0 : finiteNonNegative(tx.irrfDistribuicao),
    valorLiquidoDistribuicao: tx.valorLiquidoDistribuicao == null ? value : finiteNonNegative(tx.valorLiquidoDistribuicao),
    createdAt,
    updatedAt,
  };
}
...
export function normalizeBackup(payload) {
  ...
  const pfOtherIncomeMap = payload.pfOtherIncomeMap ?? {};
  if (!pfOtherIncomeMap || typeof pfOtherIncomeMap !== 'object' || Array.isArray(pfOtherIncomeMap)) {
    throw new Error('Outros rendimentos PF: tipo inválido.');
  }
  const normalizedOther = {};
  for (const [key, raw] of Object.entries(pfOtherIncomeMap)) {
    if (!/^\d{4}$/.test(String(key))) throw new Error(`Outros rendimentos PF: ano inválido (${key}).`);
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error(`Outros rendimentos PF: registro inválido (${key}).`);
    normalizedOther[key] = {
      descricao: typeof raw.descricao === 'string' ? raw.descricao.slice(0, 200) : '',
      valor: finiteNonNegative(raw.valor),
      irrf: finiteNonNegative(raw.irrf),
      incluirBase: raw.incluirBase !== false,
      tributavel: raw.tributavel !== false,
      irrfExclusive: finiteNonNegative(raw.irrfExclusive),
    };
  }
  ...
  return {
    ...
    pfOtherIncomeMap: normalizedOther,
