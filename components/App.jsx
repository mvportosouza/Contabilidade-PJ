...
const deriveBackupKey = async (password, salt) => {
  if (!window.crypto?.subtle) throw new Error("Criptografia segura não está disponível neste dispositivo.");
  ...
  return window.crypto.subtle.deriveKey(
    { name: BACKUP_KDF, salt, iterations: BACKUP_ITERATIONS, hash: "SHA-256" },
    material,
...
const encryptBackup = async (data, password) => {
  ...
  if (cleanPassword.length < 8) throw new Error("A senha do backup precisa ter pelo menos 8 caracteres.");

  const salt = window.crypto.getRandomValues(new Uint8Array(BACKUP_SALT_BYTES));
  const iv = window.crypto.getRandomValues(new Uint8Array(BACKUP_IV_BYTES));
  const key = await deriveBackupKey(cleanPassword, salt);
  ...
  return {
    schema: "contabilidade-pj-backup-encrypted",
    encryption: {
...
const decryptBackup = async (envelope, password) => {
  if (!envelope || envelope.schema !== "contabilidade-pj-backup-encrypted") {
    throw new Error("Este arquivo não é um backup criptografado do Contabilidade PJ.");
  }
  const enc = envelope.encryption;
  if (!enc || enc.version !== BACKUP_ENCRYPTION_VERSION || enc.algorithm !== BACKUP_ENCRYPTION_ALGORITHM || enc.kdf !== BACKUP_KDF) {
    throw new Error("Versão ou configuração de criptografia do backup não suportada.");
  }
  const cleanPassword = String(password || "");
  if (cleanPassword.length < 8) throw new Error("Informe a senha usada para criar o backup.");
...
const askBackupPassword = (mode) => {
  if (typeof window === "undefined") return "";
  const message = mode === "export"
    ? "Crie uma senha para proteger o backup.\n\nMínimo de 8 caracteres. Guarde essa senha: sem ela, o backup não poderá ser restaurado."
    : "Digite a senha usada para criar este backup criptografado.";
...
export default function App() {
  ...
  const doExport=async()=>{
    try{
      ...
      if(password.length<8){
        notify("A senha do backup precisa ter pelo menos 8 caracteres.","err");
        return;
      }
      ...
      if(typeof navigator!=="undefined" && navigator.share &&
        ...
        try{
          await navigator.share({
            title:"Backup criptografado — Contabilidade PJ",
            text:"Backup criptografado dos dados do aplicativo Contabilidade PJ.",
            files:[file],
          });
          notify("🔐 Backup criptografado gerado. Guarde a senha para restaurá-lo.","ok");
          return;
        }catch(shareError){
      ...
      notify(`🔐 Backup criptografado ${filename} gerado. Guarde a senha.` ,"ok");
    }catch(e){
  ...
  const restoreBackup=async(text)=>{
    try{
      ...
      // Novo formato: backup criptografado AES-256-GCM.
      if(raw?.schema==="contabilidade-pj-backup-encrypted"){
        const password=askBackupPassword("import");
        if(!password) return;
        source=await decryptBackup(raw,password);
      }else if(raw?.schema==="contabilidade-pj-backup"){
        // Compatibilidade com backups JSON antigos não criptografados.
        // Novos backups são sempre criptografados.
        const proceed=window.confirm(
          "Este é um backup antigo sem criptografia. Deseja restaurá-lo?\n\nRecomenda-se criar um novo backup criptografado depois da restauração."
        );
        if(!proceed) return;
      }else{
      ...
      const d=normalizeBackup(source);
      ...
      notify(e?.message||"Arquivo de backup inválido, incompatível ou senha incorreta.","err");
    }
  };
        ...
        <Modal onClose={()=>setShowSettings(false)}>
          ...
          <div style={{display:"grid",gap:10}}>
            ...
            >
              ...
              <div style={{fontSize:11,fontWeight:"normal",opacity:0.88,lineHeight:1.45}}>Selecionar um backup JSON criptografado e restaurar os dados salvos.</div>
            </button>
          </div>
          ...
          <div style={{background:"#FFF8F0",borderRadius:12,padding:"11px 13px",marginTop:14}}>
            <p style={{margin:0,fontSize:11,color:"#8A5A20",lineHeight:1.55}}>
              O backup é protegido com AES-256-GCM. A senha é necessária para restaurá-lo. Os dados passam pela validação antes de substituir os dados atuais.
            </p>
          </div>
        </Modal>
      )}
