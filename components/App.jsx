...
export default function App() {
  ...
  useEffect(() => {
    const goOnline = () => { setOnline(true); syncPendingChanges().catch(() => undefined); };
  ...
  const anexo = fator.anexo || (fator.atingiu ? 'III' : 'V');
  const rbtBase = rbt.rbt12Base || rbt.rbt12Anualizado || rbt.rbt12 || 0;
  const das = useMemo(() => calcDAS(rbtBase, receitas, anexo), [rbtBase, receitas, anexo]);
  ...
  const irrfEf = irrfMap[key] != null ? Number(irrfMap[key]) : irrf.valor;
  const ctb = Number(ctbMap[key] || 0);
  const totalObrig = das.valor + inss + irrfEf + ctb;
  const saldoAcumulado = useMemo(() => txs.reduce((s,t) => {
    if (!t?.data) return s;
    const d = new Date(`${t.data}T12:00:00`);
    const end = new Date(year, month + 1, 0, 23, 59, 59);
    if (Number.isNaN(d.getTime()) || d > end) return s;
    const value = Number(t.valor || 0);
    return s + (t.tipo === 'receita' ? value : -value);
  }, 0), [txs, year, month]);
  ...
  return <div style={{ minHeight:'100vh', maxWidth: 460, margin:'0 auto', background:C.bg, color:C.text, fontFamily:'Georgia,serif', position:'relative' }}>
    <header style={{ position:'sticky', top:0, zIndex:30, background:'rgba(242,240,237,.96)', backdropFilter:'blur(14px)', padding:'14px 16px 10px', borderBottom:`1px solid ${C.border}` }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 110px 1fr', gap:7, marginTop:10, alignItems:'center' }}>
        <select aria-label="Selecionar mês" value={month} onChange={e=>setMonth(Number(e.target.value))} style={periodSelect}>{MONTHS.map((m,i)=><option key={m} value={i}>{m}</option>)}</select>
        <select aria-label="Selecionar ano" value={year} onChange={e=>setYear(Number(e.target.value))} style={periodSelect}>{availableYears.map(y=><option key={y} value={y}>{y}</option>)}</select>
        <div style={{display:'flex',justifyContent:'flex-end',gap:5}}><button aria-label="Mês anterior" onClick={()=>{if(month===0){setMonth(11);setYear(y=>y-1)}else setMonth(m=>m-1)}} style={yearBtn}>‹</button><button aria-label="Próximo mês" onClick={()=>{if(month===11){setMonth(0);setYear(y=>y+1)}else setMonth(m=>m+1)}} style={yearBtn}>›</button></div>
      </div>
    </header>

    <main style={{ padding:'14px 14px 92px' }}>
      {tab === 'dashboard' && <Dashboard {...{receitas,despesas,resultado,saldo,brl,hideValues,das,rbt,pl,inss,irrf,irrfEf,ctb,totalObrig,fator,anexo,pendingPayables,overduePayables,openReceivables,overdueReceivables,assetBook,monthTxs, setTab, setModal, notify}} />}
...
const periodSelect = { width:'100%', border:`1px solid ${C.border}`, borderRadius:10, padding:'8px 9px', background:'#fff', color:C.navy, fontFamily:'inherit', fontSize:12, fontWeight:800, outline:'none' };
...
function Dashboard(p) {
  ...
  return <>
    ...
    <Card style={{marginBottom:10}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><Label>Saldo acumulado de lançamentos</Label><div style={big}>{val(p.saldo,p.hideValues)}</div></div><div style={{width:120,height:80}}><ResponsiveContainer><BarChart data={chart}><Bar dataKey="value" radius={[5,5,0,0]} fill={C.navyMid}/><XAxis dataKey="name" hide/><YAxis hide/><Tooltip formatter={(v)=>brl(v)}/></BarChart></ResponsiveContainer></div></div></Card>
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
      ...
      <Card><Label>Fator R</Label><div style={{...money,color:p.fator.atingiu?C.green:C.orange}}>{(p.fator.fatorR*100).toFixed(2)}%</div><div style={sub}>{p.anexo === 'III' ? 'Anexo III' : 'Anexo V'} · {p.fator.atingiu?'≥ 28%':'< 28%'}</div></Card>
    </div>
