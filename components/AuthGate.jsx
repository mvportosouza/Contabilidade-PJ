...
export default function AuthGate({ children }) {
  ...
  useEffect(() => {
     ...
     * This is important for Supabase password recovery because
     * PASSWORD_RECOVERY can be emitted while the recovery URL
     * is being processed.
     */
    ...
    const initializeAuth = async () => {
      try {
        ...
        /*
         * Fallback for recovery URLs in case PASSWORD_RECOVERY
         * was emitted before the component could receive it.
         */
        if (
          isRecoveryUrl() &&
          currentSession
  ...
  const storageMessage = () => {
    switch (storageStatus.state) {
      ...
      case 'error':
        return (
          storageStatus.error ||
          'Não foi possível sincronizar. Seus dados locais foram preservados.'
        )
      default:
        return ''
    }
  ...
  const storageVisible = Boolean(
    session &&
      ['offline', 'syncing', 'pending', 'conflict', 'error'].includes(
        storageStatus.state
      )
  )
  ...
  const requestPasswordReset = async (event) => {
    ...
    try {
      ...
      await authRequestPasswordReset(normalizedEmail)
  ...
  const updatePassword = async (event) => {
    ...
    try {
      ...
      /*
       * The password is updated first. Only then do we end the
       * recovery session and return to the login screen.
       */
      await authSignOut()
