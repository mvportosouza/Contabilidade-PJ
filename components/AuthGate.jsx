...
export default function AuthGate({ children }) {
  ...
  const requestPasswordReset = async (event) => {
    ...
    try {
      ...
      setMessage('Se o e-mail estiver cadastrado, enviaremos as instruções para redefinir sua senha.')
      setMode('login')
  ...
  if (mode === 'update-password' && recoveryActive) {
    return (
      <div style={styles.page}>
        <form onSubmit={updatePassword} style={styles.card}>
          <div style={styles.logo}>
            <img src="/assets/logo-square.png" alt="Marcus Vinícius Porto Souza" width={58} height={58} style={styles.logoImage} />
          </div>
          <h1 style={styles.title}>Finanças PJ</h1>
          <p style={styles.subtitle}>Defina uma nova senha para continuar.</p>
          <label style={styles.label}>Nova senha</label>
          <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" autoComplete="new-password" placeholder="Mínimo de 8 caracteres" style={styles.input} disabled={busy} />
          {error ? <div style={styles.error}>{error}</div> : null}
          {message ? <div style={styles.success}>{message}</div> : null}
          <button disabled={busy} type="submit" style={busy ? styles.primaryDisabled : styles.primary}>
  ...
  if (mode === 'recovery') {
    return (
      <div style={styles.page}>
        <form onSubmit={requestPasswordReset} style={styles.card}>
          <div style={styles.logo}>
            <img src="/assets/logo-square.png" alt="Marcus Vinícius Porto Souza" width={58} height={58} style={styles.logoImage} />
          </div>
          <h1 style={styles.title}>Finanças PJ</h1>
          <p style={styles.subtitle}>Informe seu e-mail para receber as instruções de redefinição.</p>
          <label style={styles.label}>E-mail</label>
          <input value={recoveryEmail} onChange={(e) => setRecoveryEmail(e.target.value)} type="email" autoComplete="email" inputMode="email" autoCapitalize="none" spellCheck={false} placeholder="seu@email.com" style={styles.input} disabled={busy} />
          {error ? <div style={styles.error}>{error}</div> : null}
          {message ? <div style={styles.success}>{message}</div> : null}
          <button disabled={busy} type="submit" style={busy ? styles.primaryDisabled : styles.primary}>
            {busy ? 'Aguarde…' : 'Enviar instruções'}
          </button>
          <button type="button" disabled={busy} onClick={() => { setMode('login'); setError(''); setMessage('') }} style={styles.linkButton}>
            Voltar para entrar
          </button>
        </form>
      </div>
