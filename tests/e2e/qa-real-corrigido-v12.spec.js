...
async function createTransaction(page, type, marker, value = '123,45', saveFavorite = false) {
  ...
  if (saveFavorite && type !== 'Distribuição de Lucro') {
    ...
    // ChkBox is a custom div, so click the actual control and wait for the
    // React state commit before submitting the form. The explicit style check
    // prevents the submit from racing the state update.
    await favoriteBox.click({ force: true })
    await expect
      .poll(
